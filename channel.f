...
!      implicit none
     ...
     &           *DEXP(-DSQRT(PI)*GYF(J))
      ...
      IF ( OASBL ) THEN
        ...
        ! SCALE PARAMETERS
        LSTAR = WINFTY * ( 1.0d0 / NU ) / RE_SUCTION

        ASTAR = LSTAR / 2.0d0 + DSQRT( LSTAR**2.0d0   + DSQRT( 
     &          LSTAR**4.0d0 + 64.0d0 * PI**2.0d0 ) ) / DSQRT( 8.0d0 )

        BSTAR = DSQRT( -LSTAR**2.0d0   + DSQRT( 
     &          LSTAR**4.0d0 + 64.0d0 * PI**2.0d0 ) ) / DSQRT( 8.0d0 )

        ! V - VELOCITY
        BU2 = - WINFTY / RE_SUCTION
        ...
        DO I = 0 , NXM
          ...
          ! U VELOCITY
          BU1(I,K,J) =  DEXP( -ASTAR * GYF(J) )
     &                * DCOS( 2.D0 * PI * TIME_RKSTEP - BSTAR * GYF(J) )

          DBU1(I,K,J) =    DEXP( -ASTAR * GYF(J) ) *
     &                  (  BSTAR * DSIN( 2.D0 * PI * TIME_RKSTEP - 
     &                                   BSTAR * GYF(J) ) 
     &                   - ASTAR * DCOS( 2.D0 * PI * TIME_RKSTEP - 
     &                                   BSTAR * GYF(J) )
     &                   )    

          ! W VELOCITY
          BU3(I,K,J) =  WINFTY * ( 1.0d0 - DEXP( -GYF(J) * WINFTY * 
     &                                    ( 1.D0 / NU ) / RE_SUCTION ) ) 

          DBU3(I,K,J) =  ( (1/NU) / RE_SUCTION ) *  WINFTY**2.0D0 * 
     &                   DEXP( -GYF(J) * WINFTY *( 1.D0 / NU ) / 
     &                   RE_SUCTION )  
...
C Do terms one at a time to save on memory
      ...
      DO I = 0      , NXM
            ...
            S1(I,K,J) =   U1(I,K,J)  * U1(I,K,J) / NU 
     &                  + BU1(I,K,J) * U1(I,K,J) / NU
      ...
      !-----------------------------------------------------------------
      ! Add the vertical derivative term explicitly u * ( v + V ) in F1
      !  
      !  ∂(u(v+V))    u(j+½) * ( v(j+½) + V ) - u(j-½) * ( v(j-½) + V ) 
      ! ---------- = ---------------------------------------------------
      !     ∂y                            ∆y
      !
      !-----------------------------------------------------------------
      ...
      DO I = 0      , NXM
        ...
        ! BU2 is the non-dimensional suction velocity BU2 = -WFS / Res
        ! Where WFS is the non dimensional free stream W velocity and
        ! Res is the suction Reynolds number

        ! Finite difference derivative in the Y direction
        S1(I,K,J) = 
     &  (   U1(I,K,J+1) * ( U2(I,K,J+1) + BU2 )  
     &    + U1(I,K,J)   * ( U2(I,K,J+1) + BU2 )
     &    - U1(I,K,J)   * ( U2(I,K,J)   + BU2 ) 
     &    - U1(I,K,J-1) * ( U2(I,K,J)   + BU2 ) ) 
     &    / ( 2.d0 * DYF(J) ) / NU
      ...
      DO I = 0      , NXM
        ...
        S1(I,K,J) = U1(I,K,J) * BU3(I,K,J) / NU
     ...
     &                  * U2(I,K,J) / NU
      ...
      !-----------------------------------------------------------------
      ! Add the vertical derivative term explicitly v * (v + V) in F2
      !-----------------------------------------------------------------
     ...
     &        - 0.25d0 *       ( U2(I,K,J  ) + U2(I,K,J-1) )**2.d0 
     &        - 0.50d0 * BU2 * ( U2(I,K,J+1) - U2(I,K,J-1) ) ) 
     &        / DY(J)  / NU
      ...
      DO K = 0 , NZM
      DO I = 0 , NXM
        ! I use the interpolated variable for ( w + W )
        S1(I,K,J) = ( (   DYF(J  ) * ( U3(I,K,J  ) + BU3(I,K,J) )
     &                  + DYF(J-1) * ( U3(I,K,J-1) + BU3(I,K,J-1) ) )  
     &                  / ( 2.d0 * DY(J) ) ) * U2(I,K,J) / NU
      ...
      !-----------------------------------------------------------------
      ! Add the vertical derivative term explicitly w * ( v + V ) in F3
      !
      !  ∂(w(v+V))    w(j+½) * ( v(j+½) + V ) - w(j-½) * ( v(j-½) + V ) 
      ! ---------- = ---------------------------------------------------
      !     ∂y                               ∆y
      !-----------------------------------------------------------------
      ...
      DO I = 0      , NXM
        ...
        S1(I,K,J) = (   U3(I,K,J+1) * ( U2(I,K,J+1) + BU2 ) 
     &                + U3(I,K,J  ) * ( U2(I,K,J+1) + BU2 )
     &                - U3(I,K,J  ) * ( U2(I,K,J  ) + BU2 )
     &                - U3(I,K,J-1) * ( U2(I,K,J  ) + BU2 ) ) 
     &                / ( 2.d0*DYF(J) ) / NU
      ...
      DO I = 0     , NXM
        ...
        S1(I,K,J) = U3(I,K,J) * ( U3(I,K,J) + BU3(I,K,J) ) / NU
      ...
      !
      !
      !   LAMINAR VERTICAL DERIVATIVES IN X AND Z MOMENTUM EQUATION 
      !
      !                                                              
      !=================================================================
      !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
     ...
     &                                * ( DBU1(I,K,J) / NU )
      ...
      !-----------------------------------------------------------------
      ! ADD ONE REMAINING TERM TO F3 WHICH IS UI.GRAD BU3 == U2 * DBU3
      ! ADD IN PHYSICAL SPACE
      ...
      DO I = 0      , NXM
        ...
        F3(I,K,J) = F3(I,K,J) - 0.5d0 * ( U2(I,K,J+1) + U2(I,K,J) )
     &                                * ( DBU3(I,K,J) / NU )
	...
	IF (RANK_G.EQ.0) THEN
     ...
     &          LSTAR**4.0d0 + 64.0d0 * PI**2.0d0 ) ) / DSQRT( 8.0d0 )
        ...
        DO I = 0 , NXM
          ...
          ! U VELOCITY
          BU1(I,K,J) =  DEXP( -ASTAR * GYF(J) )
     &                * DCOS( 2.D0 * PI * TIME - BSTAR * GYF(J) )

          DBU1(I,K,J) =    DEXP( -ASTAR * GYF(J) ) *
     &                  (  BSTAR * DSIN( 2.D0 * PI * TIME - 
     &                                   BSTAR * GYF(J) ) 
     &                   - ASTAR * DCOS( 2.D0 * PI * TIME - 
     &                                   BSTAR * GYF(J) )
     &                   )    
...
C First, construct the RHS vector,  ∂ui/∂xj * ∂uj/∂xi 
...
C Add to RHS term
      ...
      ! FACTOR OF 2 CANCELLED DUE TO INTERPOLATION ONTO GYF INCLUDING 
      ! 0.5
      DO J = 2 , NY
      DO K = 0 , TNKZ
      DO I = 0 , NKX
      ...
      ! OASBL:
      ! ------
      ! LAST TERM, BACKGROUND TERM 2*DU3/DZ*DBU3/DY == 2*DU3/DZ
      ! FACTOR OF 2 CANCELLED DUE TO INTERPOLATION ONTO GYF INCLUDING 
      ! 0.5
      IF( OASBL ) THEN
        ...
        DO I = 0 , NKX
          ...
          CF3(I,K,J) = CIKZ(K) * ( CU2(I,K,J+1) + CU2(I,K,J) )
        ...
        DO I = 0 , NXM
          ...
          F3(I,K,J) = F3(I,K,J) * DBU3(I,K,J)/NU
