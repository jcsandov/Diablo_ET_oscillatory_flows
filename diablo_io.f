...
C    CURRENT_VERSION number to make obsolete previous input files!)
     ...
     & , MOVIE, INIT_E , T0 , DISPLAY_TIME_OUTPUT_FILE
        ...
        READ(11,*)                                             ! l29
        READ(11,*)                                             ! l30
        READ(11,*) OSCILLATORY_FLOW_TYPE                       ! l31
        READ(11,*)                                             ! l32
        READ(11,*)                                             ! l33
        READ(11,*) APPLY_SYMMETRIES                            ! l34
        READ(11,*)                                             ! l35
        READ(11,*)                                             ! l36
        READ(11,*)                                             ! l37
        READ(11,*)                                             ! l38
        ...
        READ(11,*)                                             ! l43
        READ(11,*)                                             ! l44
        READ(11,*) ET_MODE_TYPE                                ! l45
        READ(11,*)                                             ! l46
        READ(11,*) KIN_E_LAM_THRS , KIN_E_TURB_THRS            ! l47
        READ(11,*)                                             ! l48
        READ(11,*) MIN_T_ES_EVAL , AVG_WINDOW_ES_EVAL          ! l49
        READ(11,*)                                             ! l50
        READ(11,*) TOL_ET_ITERATION , T_ET_COMPARISON          ! l51
        READ(11,*)                                             ! l52
        READ(11,*)                                             ! l53
        READ(11,*)                                             ! l54
        READ(11,*)                                             ! l55
        READ(11,*) RESUME_RESCALING_SIM                        ! l56
        READ(11,*)                                             ! l57
        READ(11,*) LATEST_ALPHA_E_LAM , LATEST_ALPHA_E_TURB    ! l58
        READ(11,*)                                             ! l59
        READ(11,*)                                             ! l60
        READ(11,*)                                             ! l61
        READ(11,*)                                             ! l62
        READ(11,*) RESUME_BISECTION_SIM , SAVE_ALL_BSCTN_FFS , 
     &             SET_LAMBDA_INIT                             ! l63 
        READ(11,*)                                             ! l64
        READ(11,*) LATEST_LAMBDA_LAM , LATEST_LAMBDA_TURB      ! l65
        READ(11,*)                                             ! l66
        READ(11,*) DELTA_LAMBDA_THRS, COMP_WINDOW_SPS_EVAL , 
     &             DELTA_TRAJECTORIES_THRS, ENERGY_BISECT_THRS ! l67 
        ...
        !---------------------------------------------------------------
        ! SIMULATION MODE INITIALISATION
        WALL_STOKES     = .FALSE.
        PRESSURE_STOKES = .FALSE.
        OASBL           = .FALSE.

        SELECT CASE( OSCILLATORY_FLOW_TYPE )
          CASE(1) 
            WALL_STOKES     = .TRUE.
          CASE(2) 
            PRESSURE_STOKES = .TRUE.
          CASE(3) 
            OASBL           = .TRUE.
          CASE DEFAULT 
            PRINT *, 'NO VALID INPUT FOR OSCILLATORY_FLOW_TYPE --> STOP'
            CALL END_RUN_MPI_INIT()       
        END SELECT

        !---------------------------------------------------------------
        ! EDGE TRACKING MODE INITIALISATION
        IF( ET_MODE ) THEN
          
          ET_RESCALING = .FALSE.
          ET_BISECTION = .FALSE.
          
          SELECT CASE( ET_MODE_TYPE )
            CASE(1)
              ET_RESCALING = .TRUE.
            CASE(2)
              ET_BISECTION = .TRUE.
            CASE DEFAULT 
              PRINT *, 'NO VALID INPUT FOR ET_MODE_TYPE --> STOP'
              CALL END_RUN_MPI_INIT()       
          END SELECT
        ...
        !---------------------------------------------------------------
        ! Check if it's a resumed simulation

        IF ( ET_RESCALING .AND. RESUME_RESCALING_SIM ) THEN

          ! read laminar and turbulent ALPHA values from the log files
          CALL INIT_RESUME_RESCALING()
     ...
     &           status = 'replace', action = 'write')
        ...
        IF ( ET_BISECTION .AND. RESUME_BISECTION_SIM ) THEN
     ...
     &           status = 'replace', action = 'write')
        ...
        END IF ! ET_BISECTION .AND. RESUME_BISECTION_SIM
...
C----*|--.---------.---------.---------.---------.---------.---------.-|-------|
     ...
     &             action='write')
          ...
          ! Write rescaling log file in case we need to resume the
          ! simulation
          IF( RANK_G == 0 ) CALL WRITE_RESCALING_LOG()
...
C----*|--.---------.---------.---------.---------.---------.---------.-|-------|
     ...
     &      DELTA_LAMBDA > DELTA_LAMBDA_THRS ) RETURN
        ...
        ! be controlled in diablo.f anyway, but just to make sure
        IF ( TIME - T0FILE < AVG_WINDOW_ES_EVAL ) RETURN
...
C----*|--.---------.---------.---------.---------.---------.---------.-|-------|      
      SUBROUTINE WRITE_RESCALING_LOG()
C----*|--.---------.---------.---------.---------.---------.---------.-|-------|
      
        INCLUDE 'header'

        print *, ' '
        print *, 'UPDATING rescaling.ind ...'
        print *, ' '
     ...
     &        form='formatted')
        ...
        print *, ' '
        print *, 'UPDATING rescaling.ind DONE'
        print *, ' '

      END 

C----*|--.---------.---------.---------.---------.---------.---------.-|-------|      
      SUBROUTINE INIT_RESUME_RESCALING()
C----*|--.---------.---------.---------.---------.---------.---------.-|-------|
